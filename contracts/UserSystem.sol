...
contract UserSystem {
    ...
    User[] public users;
    function setUser(/*string memory _nom,string memory _prenom,string memory _adresse,string memory _telephone,*/string memory _email,string memory _password,string memory _PK) public  returns(bool){
        require((keccak256(abi.encodePacked((list_email_user[_email].email))) == keccak256(abi.encodePacked(("")))),"erreur");
        uint id =users.push(User(0,_email,_password,_PK)) -1;
        users[id].id=id;
    ...
    function GetUser(uint _id) public view returns(uint id,string memory email,string memory password,string memory _PK){
        return (listAddresse[_id].id,listAddresse[_id].email,listAddresse[_id].password,listAddresse[_id].PK);
    }
    ...
    function setPK(uint _id,string memory _PK) public  returns(bool){
        listAddresse[_id].PK=_PK;
        return true;
    }
